...
## Scope
This file covers code inside `alco_esp/` only.
Use the root guide for repository-level workflow and packaging context.
...
## Manual verification checklist for this module
1. Start app and verify no startup errors (especially `secrets.json` and `alarm.wav` handling).
2. Confirm MQTT connect/disconnect status updates appear in UI.
3. Confirm incoming `term_d`, `term_c`, `term_k` update labels and chart.
4. Send one command per control section and confirm broker publish events.
5. Verify alarm dialog/sound behavior and reset buttons.
6. Close app and confirm graceful MQTT shutdown (no hanging process).

## Safe change patterns
- Keep signal-slot wiring explicit and close to object construction.
- For new UI controls, follow existing pattern:
  - widget creation
  - click handler
  - publish via `publishRequested`
  - status/log message
- Prefer adding helper methods over very long inline branches.
- Keep user-facing strings clear and consistent with existing Russian UI tone.
